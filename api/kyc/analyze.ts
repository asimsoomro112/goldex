import { analyzeKycDocument } from "../_services/aiService.js";
...
export default async function handler(req: any, res: any) {
  ...
  if (isRateLimited(ip, 15, 60_000)) {
    return res.status(429).json({ error: "Too many KYC analysis requests. Try again later." });
  }
  ...
  try {
    ...
    return res.status(error.statusCode || 500).json({ 
      error: error.message || "KYC document analysis failed."
    });
  }
}
